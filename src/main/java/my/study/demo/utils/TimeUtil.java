...
public class TimeUtil {
    ...
    /**
     * date类型进行格式化输出（返回类型：String）
     * demo 2019-06-11 15:00
     *
     * @param date
     * @return
     */
    public static String dateFormatTomm(Date date) {
    ...
    /**
     * 将"2019-07-02 16:01"型字符串转化为Date
     *
     * @param str
     * @return
     * @throws ParseException
     */
    public static Date StringToDateMin(String str) throws ParseException {
        SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd HH:mm");
        Date date = (Date) formatter.parse(str);
        return date;
    }
     ...
     * 2019-07-03 17:24
     */
    public static boolean CompareToDate(String d1) throws ParseException {
        System.out.println();
        boolean flag = true;
        if (new Date().compareTo(TimeUtil.StringToDateMin(d1)) < 0) {  //当前时间小于传进来的时间
            flag = true;
        } else {
            flag = false;
        }
    ...
    /**
     * 判断两个时间的先后顺序<br>
     * D1比D2小，返回true；反之为false
     */
    public static boolean CompareDateToDate(String d1, String d2) throws ParseException {
        boolean flag = true;
        if (TimeUtil.StringToDate(d1).compareTo(TimeUtil.StringToDate(d2)) < 0) {  //d1<d2
            flag = true;
        } else {
            flag = false;
        }
    ...
    /**
     * 获取两个日期之间的间隔 ，多少天多少时多少分
     *
     * @param data
     * @param nowData
     * @return
     */
    public static String getDatePoor(String data, Date nowData) throws ParseException {

        Date transfar = TimeUtil.StringToDateMin(data);
        long nd = 1000 * 24 * 60 * 60;
        long nh = 1000 * 60 * 60;
        long nm = 1000 * 60;
        // long ns = 1000;
        // 获得两个时间的毫秒时间差异
        long diff = nowData.getTime() - transfar.getTime();
        // 计算差多少天
        long day = diff / nd;
        // 计算差多少小时
     ...
     *  得到两个日期之间的天数
     */
    public static int getTwoDayByDate(Date firstDate, Date secondDate) {
        int day = 0;
        try {
            day = (int)((secondDate.getTime() - firstDate.getTime()) / (24 * 60 * 60 * 1000));
        } catch (Exception e) {
            return 0;
        }
        return day + 1; //因为毫秒关系，少一天
    }
     ...
     * @return
     */
    public static String getTwoHours(String firstString, String secondString) {
        ...
        try {
            ...
            hour = (float)(secondTime.getTime() - firstTime.getTime()) / (60 * 60 * 1000);
        } catch (Exception e) {
            return "0.00";
        }
    ...
    public static Boolean compareStringData(String str1,String str2){
        DateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        try {
            if (df.parse(str1).getTime()>df.parse(str2).getTime()){
                return true;
            }
